...
/**
 * Find a path between two points, using Dead end filler algorithm
 */
public class DeadEndFillerSolver implements Solver {
    ...
    public List<Position> solve(Maze maze, Position start, Position end) {
        ...
        for (Position deadEndPos : deadEnds) {
            ...
            while (true) {
                ...
                // If the current cell has only one reachable unfilled neighbor,
                // then it is passage to the dead end, and it should be filled
                // Otherwise we have reached fork cell, and here we can stop the cycle
        ...
        while (!currPos.equals(end)) {
            ...
            // Return an empty list if a path doesn't exist
    ...
    /**
     * Returns a reachable and unfilled neighbor, if such exists, otherwise returns an empty list
     */
